class Queue:
    ...
    def peek_last(self):  # เพิ่มเมธอด peek_last()
        if not self.is_empty():
            return self.items[-1]
        return None
...
# แสดงข้อมูลตัวสุดท้ายในคิวโดยใช้ peek_last()
last_customer = restaurant.waiting_queue.peek_last()
print(f"ลูกค้าคนสุดท้ายในคิว (peek): {last_customer}")
