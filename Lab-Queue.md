...
### ตอบคำถาม
1. Queue จะนำข้อมูลตัวแรกที่เข้ามาออกก่อน(นึกถึงธนาคารเข้าก่อนออกก่อน)(ทางเข้าทางออกทางเดียว) Stack ข้อมูลล่าสุดที่เข้ามาจะออกก่อนเข้าทีหลังออกก่อน สามารถใช้ในการเพิ่มข้อมูล (push) และการนำข้อมูลออก (pop)

2. pop(0) ใช้ลบข้อมูลตัวแรกในลิสต์ (index 0) ตามหลักการของคิวที่ทำงานแบบ FIFO ข้อมูลที่เพิ่มเข้ามาก่อน จะถูกนำออกก่อน หากใช้ pop() ในคิวจะผิดจากหลักการ FIFO เพราะ ลบข้อมูลตัวท้ายของลิสต์ ซึ่งเหมาะกับโครงสร้างแบบสแต็ก (LIFO)

3. 1.Queue ธนาคาร(ลูกค้าที่มาถึงก่อนจะได้รับบริการฝาก-ถอนเงินหรือธุรกรรมอื่น ๆ ก่อน) 
   2.Queue แอปพลิเคชันสั่งอาหาร(ร้านค้าดำเนินการตามคำสั่งซื้อในลำดับที่รับเข้ามา)
   3.Queue ซูเปอร์มาร์เก็ต(การเข้าแถวที่แคชเชียร์ ลูกค้าที่เข้าแถวก่อนจะได้รับการชำระเงินก่อน)

4. 1.สามารถใช้โมดูล heapq ใน Python เพื่อจัดการ Priority Queue ได้อย่างมีประสิทธิภาพ โดยข้อมูลจะถูกจัดเก็บในรูปแบบของ heap (โครงสร้างข้อมูลที่เรียงลำดับ)
   2.ใช้ tuple เช่น (priority_level, timestamp, item) เพื่อจัดลำดับกรณีที่ priority เท่ากันโดยใช้เวลามาเป็นตัวช่วย
   3.ในกรณีที่ต้องการให้ priority สูงสุดมีความสำคัญมากที่สุด สามารถใช้ค่า priority เป็นค่าลบ (เช่น -priority)
   4.Priority Queue ไปใช้ในสถานการณ์จริง เช่น ใช้จัดการคิวในโรงพยาบาล (ผู้ป่วยฉุกเฉินมีความสำคัญสูงสุด)
